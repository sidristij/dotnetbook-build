    ...
    public class GenericHtmlAttributesParser : InlineParser
    {
        ...
        public override bool Match(InlineProcessor processor, ref StringSlice slice)
        {
            ...
            if (TryParse(ref slice, out var attributes))
            {
                ...
                var isParagraphEmpty = paragraph != null &&
                   (paragraph.Inline.FirstChild == null && processor.Inline == null && slice.IsEmptyOrWhitespace());

                var shouldAttachToParent = paragraph?.Parent is QuoteBlock;

                if (paragraph != null && isParagraphEmpty)
                {
                    var parent = paragraph.Parent;
                    var indexOfParagraph = parent.IndexOf(paragraph);
                    if ((indexOfParagraph + 1 < parent.Count) && isParagraphEmpty)
                    {
                        objectToAttach = parent[indexOfParagraph + 1];
                        // We can remove the paragraph as it is empty
                        paragraph.RemoveAfterProcessInlines = isParagraphEmpty;
                    }
                }
                if (shouldAttachToParent)
                {
                    objectToAttach = paragraph.Parent;
                }
